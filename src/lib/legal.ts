...
export const LEGAL_DOCUMENTS = [
  ...
  {
    type: "apk_disclaimer",
    label: "Aviso sobre APKs e modificações",
    href: "/seguranca-apks",
...
export async function getLegalAcceptanceStatus(supabase: SupabaseClient, userId: string) {
  ...
  if (error) {
    return { complete: false, missing: LEGAL_DOCUMENTS.map((document) => document.type), error };
  }

  const accepted = new Map(
    (data ?? []).map((item: { document_type: string; version: string }) => [
      item.document_type,
      item.version,
    ]),
  );

  const missing = LEGAL_DOCUMENTS.filter(
    (document) => accepted.get(document.type) !== document.version,
