...
function matchesAudience(subscription: StoredSubscription, audience: NotificationAudience) {
...
export async function sendPushNotification(
  ...
  const subscriptions = ((data ?? []) as StoredSubscription[]).filter(
    (item) => matchesAudience(item, notification.audience) && categoryEnabled(item, notification.category),
  );
