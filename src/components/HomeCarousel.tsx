...
export function HomeCarousel({ slides }: Props) {
  ...
  const [player, setPlayer] = useState<{ videoId: string; title: string; youtubeUrl: string } | null>(null);
  ...
  const resolvedSlides = useMemo<ResolvedSlide[]>(() => slides.map((slide) => {
    ...
    return { ...slide, videoId: getYouTubeId(slide.actionUrl) || undefined };
  }), [feed, slides]);

  const total = resolvedSlides.length;
  useEffect(() => {
    if (paused || total < 2 || player) return;
  ...
  function openCurrent() {
    if (current.videoId) {
      setPlayer({ videoId: current.videoId, title: current.title, youtubeUrl: current.actionUrl });
      return;
    }
    ...
    <>
      ...
      >
        ...
        <div className="home-carousel__content" key={current.id}>
          ...
          {current.videoId ? (
            <button className="button button--primary" type="button" onClick={openCurrent}>
              {current.actionLabel}
              <Play size={17} />
            </button>
          ) : (
      ...
      <YouTubePlayerModal
        ...
        title={player?.title || "Vídeo do Jean na Estrada"}
