...
import { publicDriverUrl, type DriverPublicProfile } from "@/lib/driver-public";
...
export function DriverQrCard({ profile }: Props) {
  ...
  const profileUrl = useMemo(() => `${publicDriverUrl(profile.slug)}?src=qr`, [profile.slug]);
  ...
  async function share() {
    try {
      if (navigator.share) await navigator.share({ title: `Cartão profissional de ${profile.display_name}`, text: "Escaneie ou abra meu cartão para solicitar uma corrida.", url: profileUrl });
      else await copyLink();
    } catch (error) {
  ...
  function downloadQr() {
    if (!qrDataUrl) return;
    downloadDataUrl(qrDataUrl, `qr-${profile.slug}.png`);
  }
  ...
  async function downloadCard() {
    ...
    ctx.fillText("CARTÃO PROFISSIONAL", 84, 132);
    ...
    image.src = qrDataUrl;
    await new Promise<void>((resolve, reject) => { image.onload = () => resolve(); image.onerror = () => reject(new Error("QR inválido")); });
    ctx.fillStyle = "#ffffff";
    ...
    downloadDataUrl(canvas.toDataURL("image/png"), `cartao-profissional-${profile.slug}.png`);
  }

  return (
    <div className="driver-qr-layout">
      <section className="driver-qr-card-preview">
        <div className="driver-qr-card-preview__brand"><span>JNE APP</span><small>Cartão profissional</small></div>
        <div><h2>{profile.display_name}</h2><p>{profile.headline || "Motorista particular"}</p></div>
        <div className="driver-qr-image">{busy ? <LoaderCircle className="auth-spinner" size={40} /> : qrDataUrl ? <img src={qrDataUrl} alt="QR Code do perfil profissional" /> : <QrCode size={80} />}</div>
      ...
      <section className="driver-qr-actions">
        <div><span className="eyebrow">LINK PERMANENTE</span><h2>Use onde seus passageiros já estão</h2><p>O QR continua funcionando mesmo quando você altera preços, veículo ou serviços. Só não mude o endereço do perfil depois de imprimir.</p></div>
        ...
        <div className="driver-qr-action-grid">
          ...
          <a className="button button--secondary" href={`/m/${profile.slug}?src=shared_link`} target="_blank" rel="noreferrer"><ExternalLink size={18} /> Abrir perfil</a>
        </div>
        <div className="driver-qr-use-cases"><strong>Boas formas de usar</strong><ul><li>Adesivo atrás do banco</li><li>Imagem no WhatsApp Status</li><li>Cartão impresso com QR</li><li>Mensagem após uma boa corrida</li></ul></div>
