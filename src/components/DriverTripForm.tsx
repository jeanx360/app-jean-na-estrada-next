...
export function DriverTripForm({ userId, quote, reservationId }: Props) {
  ...
  async function save() {
    ...
    if (quote?.id) {
      await supabase.from("driver_quotes").update({ status: form.status === "completed" ? "completed" : "accepted", updated_at: new Date().toISOString() }).eq("id", quote.id).eq("user_id", userId);
    }
    if (reservationId) {
      await supabase.from("driver_reservations").update({ status: form.status === "completed" ? "completed" : form.status === "cancelled" ? "cancelled" : "confirmed", updated_at: new Date().toISOString() }).eq("id", reservationId).eq("driver_user_id", userId);
    }
