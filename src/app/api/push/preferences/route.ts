...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "push_enabled, general_enabled, videos_enabled, tutorials_enabled, apps_enabled, benefits_enabled, reservations_enabled",
    )
    .eq("user_id", userId)
    .maybeSingle();
  ...
  const preferences: NotificationPreferences = data
    ? {
        ...
        reservations: data.reservations_enabled,
      }
    : defaults;
  ...
  const { error } = await supabase.from("notification_preferences").upsert(
    {
      ...
      reservations_enabled: preferences.reservations,
    },
    { onConflict: "user_id" },
  );
