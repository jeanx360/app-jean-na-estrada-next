...
export default async function AdminDashboardPage() {
  ...
  return <section className="admin-dashboard-grid">{cards.map((card) => { const Icon = card.icon; return <Link className="admin-stat-card" href={card.href} key={`${card.href}-${card.label}`}><span className="admin-stat-card__icon"><Icon size={24} /></span><div><small>{card.label}</small><strong>{card.value}</strong><p>{card.detail}</p></div></Link>; })}</section>;
}
