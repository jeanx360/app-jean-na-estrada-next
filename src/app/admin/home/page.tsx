...
import { Eye, EyeOff, Images, ImageUp, Pencil, Trash2 } from "lucide-react";
import { deleteHomeSlideAction, toggleHomeSlideAction } from "@/app/admin/home/actions";
...
export const metadata: Metadata = { title: "Carrossel da página inicial" };
...
export default async function AdminHomePage({ searchParams }: Props) {
  ...
  const [slidesResult, contentsResult] = await Promise.all([
    supabase
  ...
  const initialData = edit ? slides.find((item) => item.id === edit) ?? null : null;
  ...
  return (
    <div className="admin-content-stack">
      ...
      <section className="admin-section">
        <div className="admin-section__heading">
          <div><span>{initialData ? "EDITAR DESTAQUE" : "NOVO DESTAQUE"}</span><h2><Images size={22} /> Carrossel da página inicial</h2></div>
        </div>
        <p className="admin-section__intro">
          Combine mensagens próprias, último vídeo, notícia recente e publicações cadastradas. A ordem menor aparece primeiro.
        </p>
        <AdminHomeCarouselForm initialData={initialData} contents={contents} />
      </section>
